// We have to install mysql integration with the next command
// npm install mysqljs/mysql
// Install Xamp and config with phpMyAdmin
// Setup the cypress.config.ts
// Add the requirements, setup the env


describe('Integrations with database tesing', () => {
    it.skip('Create a movie table',function (){
        cy.task('queryDb','CREATE TABLE movies(title VARCHAR(50) NOT NULL,genre VARCHAR(30) NOT NULL,director VARCHAR(60) NOT NULL,release_year INT NOT NULL,PRIMARY KEY(title))')
    });

    it.skip("Insert a movie", function () {
        cy.task(
          "queryDb",
          `INSERT INTO movies VALUES ("Joker", "psychological thriller", "Todd Phillips", 2019),("The Batman", "action", "Matt Reeves", 2022);`
        ).then((result: any) => { // Treated as an object
            expect(result.affectedRows).to.equal(2);
            expect(result.message).to.be.equal('&Records: 2  Duplicates: 0  Warnings: 0')
        });
    });

    it.skip("Select all movies", function () {
      ...
      it.skip("Update a movie", function () {
        cy.task(
          "queryDb",
      ...
      it.skip("Delete the movie table", function () {
